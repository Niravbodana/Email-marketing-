...
const SPAM_TRIGGER_PHRASES = [
  'guaranteed approval', 'instant approval', '100% approved', 'no credit check',
  'act now', 'click here', 'buy now', 'call now', 'urgent', 'congratulations',
  'risk-free', 'risk free', 'winner', 'free money', 'cash bonus', 'double your',
  'limited time', 'lowest rate', 'lowest interest', 'pre-approved', 'earn money',
  'work from home', 'no cost', 'no fees', 'eliminate debt', 'unsecured credit',
  '100% free', 'once in a lifetime', 'apply now', 'get rich', 'miracle'
];
...
function scanSpamWords(templates) {
  const hits = new Set();
  templates.forEach((t) => {
    const text = `${t.subject || ''} ${t.html || ''}`.toLowerCase();
    SPAM_TRIGGER_PHRASES.forEach((phrase) => {
...
async function computeSendingHealth({ settings, sentToday, totalAttempted, bouncedCount, suppressedCount, totalContacts, invalidCount, templates = [] }) {
  ...
  checks.push(buildCheck({
    ...
    message: !templates.length
      ...
      : spamWords.length
        ? `Yeh risky words mile: "${spamWords.slice(0, 5).join('", "')}". Inhe hata ke ya rephrase karke likho.`
